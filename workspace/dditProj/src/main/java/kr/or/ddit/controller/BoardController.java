...
public class BoardController {
	 ...
	 *	- 클래스 레벨로 요청 경로 지정 시 메서드 레벨에서 지정한 경로의 기본 경로로 취급
	...
	//속성이 하나일 때는 속성명을 생략할 수 있음. 속성ex : value... 
	@RequestMapping("/modify") // modify: 수정,변경 하다
	...
	//http://localhost:8090/board/read/detail?boardNo=7 (before, AsIs)
	// * 경로 패턴 매핑
	// 요청 경로를 동적으로 표현이 가능한 경로 패턴을 지정할 수 있음
	// 경로 변수에 해당하는 값을 파라미터 변수에 설정할 수 있음.
	//http://localhost:8090/board/read/detail/boardNo/7	(after, ToBe)
