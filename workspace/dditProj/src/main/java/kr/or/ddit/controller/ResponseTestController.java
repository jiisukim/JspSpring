...
public class ResponseTestController {
   ...
   //1. return타입이 void라면.. 요청경로(/board/goHome0101)와 동일한 뷰(/board/goHome0101.jsp)를 가리킴
   ...
   public ModelAndView home0301() {
	   ...
	   //Responsebody 어노테이션 사용 후 jsp 에서는
	   //Member[userName=김은대,password=1234]
	   //Responsebody 어노테이션 사용 안함에도 결과가 같은 이유는?
   ...
   @ResponseBody
   @RequestMapping(value="/board/home0401", method=RequestMethod.GET)
   public ModelAndView home0401(ModelAndView mav){
      ...
      logger.info("객체비교 : " + (member == member2));
      logger.info("객체비교 : " + (member.equals(member2)));
      
      mav.addObject("list", list);
      //forwarding
      mav.setViewName("board/home0401");
      return mav;
   ...
   // response 할 때 Http 헤더 정보와 문자열 데이터를 전달하는 용도로 사용
   ...
   // response 할때 Http 헤더 정보와 데이터를 전달하는 용도로 사용
