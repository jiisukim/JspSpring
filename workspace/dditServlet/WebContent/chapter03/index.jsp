...
/*
	...
	URL에 포함시키려면 인코딩을 해줘야함
	- response.sendRedirect() 메서드를 사용하는 경우에도 마찬가지로 인코딩한 쿼리 문자열(?name = 개똥이)을 사용해야 함.
	- Java.net.URLEncoder 클래스가 존재하고, URLEncoder.encode() 메서드를 사용하면 파라미터 값으로 사용된 문자열을 지정한 캐릭터 셋(utf-8)으로 인코딩 할 수 있다. 
	- 크롬, 파이어폭스는 웹 브라우저가 UTF-8로 인코딩된 주소 경로나 파라미터 값을 해당 문자로 변환해줌
	- IE(익스플로어)는 인코딩된 파라미터 값을 그대로 보여줌
*/
